...
- `readTabMeter(connection, vaultPda)` — READ-ONLY reporter: `{ spent, currentOutstanding, maxRevolvingCapacity, remaining }`. Reports remaining capacity; NEVER refuses. On-chain `RevolvingCapacityExceeded` stays the authoritative guard. (A client-side refuser would invite a stale-cache TOCTOU bug — explicitly out of scope, see §6.)
